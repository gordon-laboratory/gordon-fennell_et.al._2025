...
add_scale_bar <- function(plt, origin, x_len, y_len) {
  # Create data frame for the horizontal scale bar
  scale_bar_x <- data.frame(
    x = c(origin[1], origin[1] + x_len),
    y = c(origin[2], origin[2]),
    facet = factor(1)
  )
  
  # Create data frame for the vertical scale bar
  scale_bar_y <- data.frame(
    x = c(origin[1], origin[1]),
    y = c(origin[2], origin[2] + y_len),
    facet = factor(1)
  )
...
plt_hsd_matrix <- function(df_pairs, comp, heat_limits = NA){
  n_contrasts <- df_pairs %>%
    pull(!!as.name(str_c('contrast1_', comp))) %>%
    unique() %>%
    length()
  
  df_pairs <- df_pairs %>%
    rename(temp_var1 = !!as.name(str_c('contrast1_', comp)), 
           temp_var2 = !!as.name(str_c('contrast2_', comp))) %>%
    rename( !!as.name(str_c('contrast2_', comp)) := temp_var1, 
            !!as.name(str_c('contrast1_', comp)) := temp_var2
    ) %>%
  ...
  plt <- df_pairs %>%
    ...
    ggplot(aes(!!as.name(str_c('contrast2_', comp)), !!as.name(str_c('contrast1_', comp)), fill = effect_size, label = sig_symbol)) +
    geom_tile() +
    geom_text() +
    theme_ag01() +
