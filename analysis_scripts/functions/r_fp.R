...
fp_moving_average <- function(streams_data, streams_info, smoothing_window) {
  ...
  if(smoothing_window > 0){ # if smoothing...
    ...
    channel_ids  <- streams_info %>%
      select(blockname, name, fs) %>% unique()
    ...
    for(channel_id_index in seq(1, dim(channel_ids)[1])){ # for each channel...
      ...
      streams_data_loop <- streams_data %>%
        filter(blockname == channel_ids$blockname[channel_id_index],
               channel   == channel_ids$name[channel_id_index])

        # calculate moving average
      streams_data_loop <- streams_data_loop %>%
        mutate(au_smooth_ma = rollmean  (x = raw_au,                                                       # variable to be smoothed
                                         ...
                                         na.pad = TRUE))                                                   # pad to maintain vector length
...
fp_moving_zscore <-  function(streams, pre_window) {
  ...
  temp_moving_zscore <- streams %>% filter(blockname != blockname) %>% select(blockname, channel_id, time)
  ...
  for(n_stream in 1:nrow(streams_to_process)){
    ...
    for(n_sample in 1+pre_window:nrow(temp_stream)){
      temp_loop <- temp_stream[(n_sample - pre_window) : (n_sample - 1),] %>%
        ...
        summarise(pre_window_mu = mean(delta_signal_poly_zscore),
                  pre_window_sd = sd(delta_signal_poly_zscore)) %>%

        mutate(blockname = temp_stream$blockname[n_sample],
...
fp_normalization_npm <- function(streams_data_smoothed, log_npm){
  ...
  f_fp_normalization_npm <- function(df, log_npm){
    ...
    if (exists('control', where = df)){control <- df %>% pull(control)}
    time    <- df %>% pull(time)

    # extract parameters
    blockname_function <- df$blockname_temp %>% unique()
    poly_degree_fitted_control <- log_npm %>% filter(blockname == blockname_function) %>% pull(poly_degree_fitted_control) %>% unique() 
    poly_degree_polyfit        <- log_npm %>% filter(blockname == blockname_function) %>% pull(poly_degree_polyfit)        %>% unique()
    toggle_baseline_fit        <- log_npm %>% filter(blockname == blockname_function) %>% pull(baseline_fit)               %>% unique()
    tm_baseline                <- log_npm %>% filter(blockname == blockname_function) %>% pull(tm_baseline)                %>% unique()

    if(toggle_baseline_fit == 0){
      df_fitting_subset <- df
    } else {
      df_fitting_subset <- df %>%
        filter(time < min(time) + tm_baseline)
    }

    # generate fitted 405 signal
    if (exists('control', where = df)){
      fit_signal_control <- lm(signal ~ poly(control, poly_degree_fitted_control, raw=TRUE), data = df_fitting_subset)
      control_fitted     <-  predict(fit_signal_control, newdata = df)
    }

    # generate polynomial fit for signal
    fit_poly_signal <- lm(signal~poly(time, poly_degree_polyfit), data = df_fitting_subset)
    poly_signal     <- predict(fit_poly_signal, newdata = df)

    # generate polynomial fit for control
    if (exists('control', where = df)){
      fit_poly_control <- lm(control~poly(time, poly_degree_polyfit), data = df_fitting_subset)
      poly_control     <- predict(fit_poly_control, newdata = df)
    }
  ...
  # join control to signal (spreads control over all signals within dataset)
  streams_data_smoothed_fitted <- left_join(
    streams_data_smoothed_fitted %>%
      filter(channel_id != 'control') %>%
      select(-channel_id) %>%
      rename(signal = au_smooth_ma),
    
    streams_data_smoothed_fitted %>%
      filter(channel_id == 'control') %>%
      select(-channel_id, -channel_wavelength) %>%
      rename(control = au_smooth_ma), 
    
    by = join_by(session_name, blockname, branch_id, time, cycle_number)
  )
  
  
  if (exists('control', where = streams_data_smoothed_fitted)){
    streams_data_smoothed_fitted <- streams_data_smoothed_fitted %>%
...
fp_downsample <- function(streams_data_smoothed_fitted, fs, down_sampled_freq){
  ...
  tm_bins <- seq(min(streams_data_smoothed_fitted$time) %>% round(2),
                 max(streams_data_smoothed_fitted$time) %>% round(2),
                 1/down_sampled_freq)# generate time bins and set labels to end of each window
...
timeseries_resample <- function(df, var_time, var_signal) {
  ...
  df_resampled <- df %>%
    ...
    mutate(!!as.name(var_time) := !!as.name(var_time) %>% as.character() %>% as.double()) %>%
...
fp_filter_events <- function(events, fp_events_of_interest){
  ...
  if(sum(str_detect(fp_events_of_interest, 'pump_onset')) > 0){
    bout_onset <- events %>%  filter(event_id_char == 'sol_onset') %>% mutate(event_id_char = 'pump_onset')
    events_filtered <- c(events_filtered, list(bout_onset))
  }
  ...
  if(sum(str_detect(fp_events_of_interest, 'trial_start')) > 0){
    bout_onset <- events %>%  filter(event_id_char == 'trial_start')
    events_filtered <- c(events_filtered, list(bout_onset))
  }
  ...
  if(sum(str_detect(fp_events_of_interest, 'laser_onset')) > 0){
    bout_onset <- events %>%  filter(event_id_char == 'laser_onset')
    events_filtered <- c(events_filtered, list(bout_onset))
  }

  if(sum(str_detect(fp_events_of_interest, 'laser_offset')) > 0){
    bout_onset <- events %>%  filter(event_id_char == 'laser_offset')
    events_filtered <- c(events_filtered, list(bout_onset))
  }
...
fp_preprocess <- function(dir_extracted, dir_processed, log_fp, manual_experiments = NA, manual_blocknames = NA, overwrite = 0, dir_extracted_arduino = NA) {
  ...
  suffixes <- c('_streams_peth.feather',
  ...
  for (n_file in seq(1,dim(key_files)[1])){
    ...
    streams_data_smoothed <-
      fp_moving_average(streams_data,
                        streams_info,
    ...
    for(n_signal in 1:nrow(signal_ids)){
      ...
      streams_data_smoothed_fitted_loop <-
        fp_streams_fitted(streams_data               = streams_data_loop,
                          streams_info               = streams_info,
        ...
        mutate(signal_wavelength  = loop_signal_id$channel_wavelength,
    ...
    streams_data_smoothed_fitted_downsampled <-
      fp_downsample(streams_data_smoothed_fitted,
                    fs,
    ...
    if(flag_events_filtered){
        ...
        streams_data_smoothed_fitted_peth_downsampled <-
          fp_peri_event_time_histogram(streams_data_smoothed_fitted,
                                       events_filtered,
                                       key_file$peth_pre,
                                       key_file$peth_post,
                                       fs,
  ...
  for(blockname_loop in blocknames){ # for each blockname
    ...
    for(n_set in seq(1, rotation_event_sets_loop %>% pull(response_set) %>% max(na.rm = TRUE))){ # for each event set
      ...
      rescale_streams_resampled <- rescale_streams_resampled %>%
        ...
        select(blockname, fiber_id, signal_wavelength, time_rel, time:delta_control_poly_dff) %>% #*** variables defined here ***
        group_by(blockname, fiber_id, signal_wavelength, time_rel) %>%
        summarise_all(mean, .groups = 'drop') %>%                                    # summarise all variables using mean
        mutate(time_rel = time_rel %>% as.character() %>% as.double()) %>%           # convert time to numeric
        filter(time_rel < max(time_rel)) %>%
...
npm_extract <- function(log_npm, dir_npm_raw, dir_npm_extracted, manual_session_names, overwrite, filter_red_opto_sd){
  ...
  for(session_name_process in sessions_to_process$session_name){  # loop over each session
    ...
    var_join <- c("session_name", "blockname", "branch_id", "region", "down_sampled_freq", "channel_number", "roi_set")

    log_npm_filtered_to_channels <- log_npm %>%
      filter(session_name == session_name_process) %>%
      select(session_name, blockname, roi_set, branch_id, region, down_sampled_freq, all_of(var_channels))

    for (var_channel in var_channels){
      loop_df <- log_npm_filtered_to_channels %>%
        select(session_name, blockname, roi_set, branch_id, region, down_sampled_freq, !!as.name(var_channel)) %>%
      ...
      if(var_channel == var_channels[1]){
        ...
        suppressMessages(
          log_npm_long <- loop_df %>%
            left_join(log_npm_long, .)
        )
      }
    }
    

    # set roi based on branch id and color (ROIs should be set on red then green (e.g. roi 0 and roi 1 for branch red and branch green))
    # if roi_set is not dual, then set roi = branch id - 1
    ...
    if(flag_clock){ # if there is a clock noted in log_npm
      ...
      suppressMessages(
        data_npm_serial_clock <- data_npm_serial_clock %>%
    ...
    if(flag_clock){
      ...
      suppressMessages(
        filter_times <- data_npm_fp_long_filtered %>%
          ...
          group_by(session_name, blockname) %>%
          filter(ts_comp > min(ts_comp) + tm_trim_start * 1000) %>%
          filter(ts_comp < max(ts_comp) - tm_trim_end * 1000) %>% 
          filter(ts_comp == min(ts_comp) | ts_comp == max(ts_comp)) %>%
          mutate(break_id = ifelse(ts_comp == min(ts_comp), 'tm_session_start', 'tm_session_end')) %>%
          ungroup() %>%
    ...
    plt_starts <- data_npm_fp_long_filtered_resampled %>%
      ...
      geom_line() +
      geom_vline(data = filter_times %>% filter(break_id == 'tm_session_start'),
                 aes(xintercept = (ts_comp - ts_comp_subtract)/1000/60,
                     linetype = blockname)) +
      theme_minimal() +
      xlab('time (m)') +
      ylab('mean raw (au)') +
...
fp_preprocess_npm <-
  function(dir_npm_extracted,
    ...
    for (session_process in sessions_to_process) {
      ...
      if (flag_clock) {
        session_clock <-
          arrow::read_feather(str_c(dir_npm_extracted,
                                    '/',
                                    session_process,
                                    '_clock.feather'))  %>%
          mutate(ts_comp = ts_comp / 1000)
      ...
      if (length(events_manual_fns) == 0) {
        ...
        events_manual <- bind_rows(events_manual %>%
        ...
        for (blockname_process in blocknames) {
          ...
          if (nrow(events_filtered_loop) == 0) {
            ...
            if (blockname_process == blocknames[1]) {
              ...
              streams_data_smoothed_fitted_peth_downsampled <-
                streams_data_smoothed_fitted_peth_downsampled_loop %>%
                bind_rows(streams_data_smoothed_fitted_peth_downsampled,
                          .)
            }
          }
        }
...
return_channel_config <- function(streams){
  ...
  num_fiber_id           <- channel_info %>% select(fiber_id) %>% unique() %>% nrow()
  num_signal_wavelength  <- channel_info %>% select(signal_wavelength) %>% unique() %>% nrow()
